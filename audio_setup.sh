...
echo "# Последнее обновление: $(date)" >> "$AUDIO_CONF"
...
EOF
    ...
    # Добавляем скрипт обнаружения в автозапуск i3
    ...
    # Запускаем скрипт определения клавиш прямо сейчас
    if [ -n "$DISPLAY" ]; then
        ...
        echo -e "${YELLOW}⚠️ DISPLAY не определен, пропускаем настройку клавиш${RESET}"
    fi
